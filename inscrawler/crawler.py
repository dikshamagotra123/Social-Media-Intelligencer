...
from . import secret  # Contains Username and Password
...
class Logging(object):
    ...
    def __init__(self):
        try:
            ...
            self.log_disable = False  # False because the loggers info should be entered if try runs.
        except Exception:
            self.log_disable = True  # Should not save info about any failed logging attempt.

    def cleanup(self, timestamp):  # Function to clean logs if it is more than 7 days.
        days = 86400 * 7
        days_ago_log = "/tmp/%s-%s.log" % (Logging.PREFIX, timestamp - days)
        for log in glob.glob("/tmp/instagram-crawler-*.log"):
            if log < days_ago_log:
                os.remove(log)  # Removing the log greater than 7 days.
...
class InsCrawler(Logging):
    ...
    def login(self):
        ...
        login_btn = browser.find_one("._acan")
        login_btn.click()
    ...
    def get_user_profile_from_script_shared_data(self, username):
        ...
        url = "%s/%s/" % (InsCrawler.URL, username)
        browser.get(url)
        source = browser.driver.page_source # Page ka source
        p = re.compile(r"window._sharedData = (?P<json>.*?);</script>", re.DOTALL)  # window._sharedData contains
        # profile content and converts it into json format (AFTER CAMBRIDGE ANALYTICA CASE)
        # .(Regular Expression to compile pattern.) (Combines dot-all)
        json_data = re.search(p, source).group("json")
        data = json.loads(json_data)  # used to parse a valid JSON string and convert it into a Python Dictionary.
    ...
    def auto_like(self, tag="", maximum=1000):
        ...
        ele_post = browser.find_one('._mck9w a')
        ele_post.click()
        ...
        def start_fetching(pre_post_num, wait_time):
            ...
            for ele in ele_posts:
                ...
                if key not in key_set:
                    dict_post = { "key": key }
                    ele_img = browser.find_one("._aagv img", ele)
